...
/**
 * 平日スケジュールから割り当てを取得
 * @param {string} section セクション名でフィルタ（nullなら全件）
 * @param {string} dateStr 日付でフィルタ（nullなら全件）
 * @return {Array} [{date, slot_id, slot_name, doctor_id, doctor_name, section}, ...]
 */
function getWeekdayAssignments(ssOp, yearMonth, section, dateStr) {
  var sheet = getSheet(ssOp, "平日スケジュール_" + yearMonth);
  if (!sheet) return [];
  ...
  for (var i = 1; i < data.length; i++) {
    ...
    if (section && String(row.section) !== section) continue;
...
function sendWeekdayScheduleConfirmed(data) {
  ...
  for (var m = 0; m < yearMonths.length; m++) {
    var assignments = getWeekdayAssignments(ssOp, yearMonths[m], section, null);
...
function checkWeekdayDeadlines() {
  ...
  for (var i = 0; i < configs.length; i++) {
    ...
    // 入力済み医員を取得
    var prefSheet = getSheet(ssOp, "平日希望_" + section);
    var submittedIds = {};
    if (prefSheet) {
      var prefData = prefSheet.getDataRange().getValues();
      if (prefData.length > 1) {
 ...
 * トリガー設定: 日ベースのタイマー（毎日 18:00-19:00 推奨）
 */
function sendWeekdayDayBeforeReminder() {
  ...
  for (var i = 0; i < configs.length; i++) {
    ...
    var assignments = getWeekdayAssignments(ssOp, yearMonth, cfg.section, tomorrowStr);
