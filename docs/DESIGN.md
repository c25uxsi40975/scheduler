...
| 土曜運用データ | 希望_YYYY-MM / スケジュール_YYYY-MM | `spreadsheet_key_operational` |
...
| 5 | 必須メンバー | 優先度「必須」の医員×外勤先は月1回以上割り当て |
| 6 | 限定メンバー | 限定メンバーが設定された外勤先には、リスト内の医員のみ割り当て可能 |
| 6b | 除外メンバー | 除外メンバーに指定された医員はその外勤先に割り当て不可 |
...
1. コスト行列を構築:
   ...
   - NG日・×外勤先・限定メンバー外・除外メンバー → cost = INF (1e9)
2. `linear_sum_assignment(cost_matrix)` で最適マッチング
3. `max_assignments` 超過チェック（日をまたいで累積）
...
- 限定メンバー（ホワイトリスト）・除外メンバー（ブラックリスト）・必須（月1回以上）・×禁止・△ペナルティ・月回数上限
- 日別外勤先希望ボーナス
- 3モード×1回の一括プラン生成
...
- 制約の尊重（NG日、×外勤先、限定メンバー、除外メンバー、月回数上限）
...
- **admin_master.py**: 希望入力対象月設定、医員の追加/編集/削除（ID・アカウント・パスワード・メール・回数上限・役職）、外勤先の追加/編集（テンプレート選択対応・日当・頻度・労力コスト・勤務時間・時間帯・勤務地）、指名・限定メンバー・除外メンバー・優先度設定、日別外勤先希望、日別設定
